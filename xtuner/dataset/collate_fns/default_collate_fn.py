...
# <<<<<<< HEAD:xtuner/dataset/collate_fns/default_collate_fn.py
#     seq_parallel_world_size = get_sequence_parallel_world_size()
#
# =======
# >>>>>>> pr-460:xtuner/dataset/collate_fns/defalut_collate_fn.py
    seq_parallel_world_size = get_sequence_parallel_world_size()
    input_ids, labels = [], []
...
def anyshape_llava_collate_fn(instances: Sequence[Dict],
                              pad_index: int = DEFAULT_PAD_TOKEN_INDEX,
                              return_hf_format: bool = False,
                              use_varlen_attn: bool = False):
    input_ids, labels = [], []
    ...
    for example in instances:
        ...
        if has_image:
            pixel_values.append(example['pixel_values'])
            orig_sizes.append(example.get('orig_size', None))
